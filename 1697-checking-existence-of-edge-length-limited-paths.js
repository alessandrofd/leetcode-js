...
const distanceLimitedPathsExist = (n, edgeList, queries) => {
  ...
  const sortedEdges = [...edgeList].sort(([, , a], [, , b]) => a - b)
  ...
  for (const query of sortedQueries) {
    ;[orig, dest, limit, resultIndex] = query
    while (
      edgesIndex < sortedEdges.length &&
      sortedEdges[edgesIndex][2] < limit
    ) {
      ;[i, j, _] = sortedEdges[edgesIndex++]
      union(i, j)
    }
    result[resultIndex] = find(orig) === find(dest)
  }
