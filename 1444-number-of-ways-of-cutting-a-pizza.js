...
const ways = (pizza, k) => {
  ...
  const apples = new Array(rows)
    .fill()
    .map((_, row) =>
      new Array(cols).fill().map((_, col) => (pizza[row][col] === 'A' ? 1 : 0))
    )
  const sumApples = new NumMatrix(apples)

  const dp = new Array(k)
    .fill()
    .map((_) => new Array(rows).fill().map((_) => new Array(cols).fill(0)))

  // Caso base - não tem trasição, basta garantir que o pedaço de pizza contenha
  // pelo menos uma maçã
  for (let row = 0; row < rows; row++)
    for (let col = 0; col < cols; col++)
      dp[0][row][col] = sumApples.sumRegion(row, col) > 0 ? 1 : 0
  ...
  for (let remain = 1; remain < k; remain++)
    for (let row = 0; row < rows; row++)
      for (let col = 0; col < cols; col++) {
        let validCuts = 0
        // Corte horizontal
        for (let newRow = row + 1; newRow < rows; newRow++)
          if (sumApples.sumRegion(row, col, newRow - 1, cols - 1) > 0)
            validCuts += dp[remain - 1][newRow][col]

        // Corte vertical
        for (let newCol = col + 1; newCol < cols; newCol++)
          if (sumApples.sumRegion(row, col, rows - 1, newCol - 1) > 0)
            validCuts += dp[remain - 1][row][newCol]

        dp[remain][row][col] += validCuts % MOD
      }
