...
const leftMostColumnWithOne_binSearch = (binMatrix) => {
  // const leftMostColumnWithOne = (binMatrix) => {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    if (binMatrix.get(row, lo) === 1) leftMost = Math.min(leftMost, lo)
  }
  return leftMost === cols ? -1 : leftMost
}

// Approach 3: Start at Top Right, Move Only Left and Down
// const leftMostColumnWithOne_downAndLeft = (binMatrix) => {
const leftMostColumnWithOne = (binMatrix) => {
  const [rows, cols] = binMatrix.dimensions()
  ...
  while (row < rows && col >= 0) {
    if (binMatrix.get(row, col) === 0) row++
    else col--
  }
...
console.log(leftMostColumnWithOne(new BinaryMatrix(mat)))
