 ...
 * @return {number}
 */
const maximumScore_bin_search = (nums, k) => {
  /**
  ...
  const solve = (nums, k) => {
    ...
    for (let j = 0; j < right.length; j++) {
      const leftLen = left.length - bisectLeft(left, right[j])
      const rightLen = j + 1
      const len = leftLen + rightLen
      maxScore = Math.max(maxScore, right[j] * len)
    }
    return maxScore
  }

  return Math.max(solve(nums, k), solve(nums.reverse(), n - k - 1))
}
 ...
 * @return {number}
 */
const maximumScore_monotonic_stack = (nums, k) => {
  /**
 ...
 * @return {number}
 */
const maximumScore_greedy = (nums, k) => {
  /**
   * Approach 3: Greedy
   *
  ...
  while (left > 0 || right < n - 1) {
    const leftNum = left === 0 ? 0 : nums[left - 1]
    const rightNum = right === n - 1 ? 0 : nums[right + 1]
...
// prettier-ignore
const funcs = [
  // maximumScore_bin_search,
  // maximumScore_monotonic_stack,
  maximumScore_greedy,
]
