 ...
 * @return {boolean}
 */
const find132pattern_stack_prefix_min = (nums) => {
  // Pilha com prefixMin
  ...
  let min = Infinity
  const prefixMin = nums.map((n) => (min = Math.min(min, n)))

  const stack = []

  for (let i = n - 1; i >= 0; i--) {
    while (stack.length && stack.at(-1) <= prefixMin[i]) {
      stack.pop()
    }
    if (stack.length && nums[i] > stack.at(-1)) {
      return true
    }
...
const funcs = [
  ...
  find132pattern_stack_prefix_min,
