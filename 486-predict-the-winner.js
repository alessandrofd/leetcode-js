 ...
 * @return {boolean}
 */
const PredictTheWinner_top_down_dp = (nums) => {
  const n = nums.length
  const dp = new Array(n).fill().map((_) => new Array(n))

  const recurse = (left, right) => {
    if (left === right) return nums[left]
    if (dp[left][right]) return dp[left][right]

    const chooseLeft = nums[left] - recurse(left + 1, right)
    const chooseRight = nums[right] - recurse(left, right - 1)
 ...
 * @return {boolean}
 */
const PredictTheWinner_bottom_up_dp = (nums) => {
  const n = nums.length
  ...
  for (let diff = 1; diff < n; diff++) {
    for (let left = 0; left < n - diff; left++) {
      ...
      const chooseLeft = nums[left] - dp[left + 1][right]
      const chooseRight = nums[right] - dp[left][right - 1]
 ...
 * @return {boolean}
 */
const PredictTheWinner_bottom_up_dp_space_optimized = (nums) => {
  const n = nums.length
  const diag = [...nums]
  ...
  for (let diff = 1; diff < n; diff++) {
    for (let left = 0; left < n - diff; left++) {
      const right = left + diff
      const chooseLeft = nums[left] - diag[left + 1]
      const chooseRight = nums[right] - diag[left]
      diag[left] = Math.max(chooseLeft, chooseRight)
    }
  }
...
console.log(PredictTheWinner_top_down_dp(nums))
console.log(PredictTheWinner_bottom_up_dp(nums))
console.log(PredictTheWinner_bottom_up_dp_space_optimized(nums))
