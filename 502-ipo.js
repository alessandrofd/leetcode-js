...
const findMaximizedCapital = (k, w, profits, capital) => {
  ...
  const sorted = [...capital].map((c, i) => [i, c]).sort((a, b) => a[1] - b[1])

  const queue = new MaxPriorityQueue({ priority: (element) => element })

  let ptr = 0
  while (ptr < n && sorted[ptr][1] <= w) {
    queue.enqueue(profits[sorted[ptr][0]])
    ptr++
  }
