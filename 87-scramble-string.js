...
const isScramble_dp = (s1, s2) => {
  ...
  const dp = new Array(n + 1)
    .fill()
    .map((_) => new Array(n).fill().map((_) => new Array(n).fill(false)))
  ...
  for (let length = 2; length <= n; length++) {
    for (let i = 0; i <= n - length; i++) {
      for (let j = 0; j <= n - length; j++) {
        for (let slice = 1; slice < length; slice++) {
          dp[length][i][j] ||=
            dp[slice][i][j] && dp[length - slice][i + slice][j + slice]
          dp[length][i][j] ||=
            dp[slice][i][j + length - slice] && dp[length - slice][i + slice][j]
        }
      }
    }
  }
...
const isScramble_dfs = (s1, s2) => {
  ...
  const recurse = (s1, s2) => {
    ...
    for (let i = 1; i < n; i++) {
      const withoutSwap =
        recurse(s1.slice(0, i), s2.slice(0, i)) &&
        recurse(s1.slice(i), s2.slice(i))
      ...
      const withSwap =
        recurse(s1.slice(0, i), s2.slice(n - i)) &&
        recurse(s1.slice(i), s2.slice(0, n - i))
