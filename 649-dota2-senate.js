 ...
 * @return {string}
 */
const predictPartyVictory_queue = (senate) => {
  const numSenators = senate.length
  const queue = [...senate]
  let numRadiant = queue.filter((s) => s == 'R').length
  let numDire = numSenators - numRadiant
...
const predictPartyVictory_stack = (senate) => {
  const senators = [...senate]
  const stack = []

  for (const senator of senators) {
    if (!stack.length || stack.at(-1) == senator) stack.push(senator)
    else senators.push(stack.pop())
  }
...
console.log(predictPartyVictory_stack(senate))
