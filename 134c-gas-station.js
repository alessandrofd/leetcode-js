...
const canCompleteCircuit = (gas, cost) => {
  const numGasStations = gas.length
  let startingStation = 0
  let gasTank = 0
  let finalGasTank = 0

  for (let gasStation = 0; gasStation < numGasStations; gasStation++) {
    finalGasTank += gas[gasStation] - cost[gasStation]
    gasTank += gas[gasStation] - cost[gasStation]
    if (gasTank < 0) {
      gasTank = 0
      startingStation = gasStation + 1
    }
  }

  return finalGasTank >= 0 ? startingStation : -1
}
