 ...
 * @return {number}
 */
const constrainedSubsetSum_pq = (nums, k) => {
  /**
  ...
  const pq = new MaxPriorityQueue({ priority: (element) => element[0] })
  pq.enqueue([nums[0], 0])

  let maxSum = nums[0]
  for (let i = 1; i < nums.length; i++) {
    while (pq.size() > 0 && pq.front().element[1] < i - k) pq.dequeue()

    let curr = nums[i]
    if (pq.size() > 0) curr += Math.max(0, pq.front().element[0])
    pq.enqueue([curr, i])
 ...
 * @return {number}
 */
const constrainedSubsetSum_treeMap = (nums, k) => {
  /**
 ...
 * @return {number}
 */
const constrainedSubsetSum_monotonic_deque = (nums, k) => {
  /**
  ...
  for (let i = 0; i < n; i++) {
    ...
    dp[i] = nums[i] + (queue.length > 0 ? dp[queue[0]] : 0)

    while (queue.length > 0 && dp[queue.at(-1)] < dp[i]) queue.pop()
...
// prettier-ignore
const funcs = [
  // constrainedSubsetSum_pq,
  // constrainedSubsetSum_treeMap,
  constrainedSubsetSum_monotonic_deque,
]
