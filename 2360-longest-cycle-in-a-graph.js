 ...
 *
 * O fato do nó participar de apenas um ciclo permite que mantenhamos uma lista
...
const longestCycle_dfs = (edges) => {
  ...
  for (let start = 0; start < numNodes; start++) {
    ...
    while (node !== -1) {
      ...
      if (path.has(node)) {
        longest = Math.max(longest, count - path.get(node))
        break
      }
      path.set(node, count++)
      node = edges[node]
    }
    for (visitedNode of path.keys()) visited.add(visitedNode)
  }
 ...
 * @return {number}
 */
const longestCycle_dfs_array = (edges) => {
  const numNodes = edges.length
  ...
  for (let start = 0; start < numNodes; start++) {
    ...
    while (node !== -1) {
      if (visited[node]) {
        if (visited[node] >= startPath) {
          longest = Math.max(longest, count - visited[node])
        }
        break
      }
      visited[node] = count++
...
const longestCycle_kahn = (edges) => {
  const numNodes = edges.length

  const degrees = new Array(numNodes).fill(0)
  for (node of edges) if (node !== -1) degrees[node]++

  const queue = []
  degrees.forEach((nodes, i) => {
    if (nodes === 0) queue.push(i)
  })
  ...
  // Processamento dos ciclos - subproduto do algoritmo de Kahn
...
console.log(longestCycle_dfs(edges))
console.log(longestCycle_dfs_array(edges))
console.log(longestCycle_kahn(edges))
