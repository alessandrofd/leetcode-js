 ...
 * @return {number}
 */
const minimumHealth_maxReduce = (damage, armor) => {
  const protection = Math.min(armor, Math.max(...damage))
  const health = damage.reduce((acc, nxt) => acc + nxt) - protection + 1
  return health
}

const minimumHealth = (damage, armor) => {
  let totalDamage = 0
  let maxDamage = 0
