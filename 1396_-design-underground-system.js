...
class UndergroundSystem {
   ...
   * @return {void}
   */
  checkIn(id, startStation, departure) {
    this.passengers.set(id, { startStation, departure })
  }
   ...
   * @return {void}
   */
  checkOut(id, endStation, arrival) {
    const { startStation, departure } = this.passengers.get(id)

    if (!this.trips.get(startStation)) this.trips.set(startStation, new Map())

    const destinations = this.trips.get(startStation)
    if (!destinations.get(endStation)) destinations.set(endStation, [])

    const travelTimes = destinations.get(endStation)
    travelTimes.push(arrival - departure)
  }
  ...
  getAverageTime(startStation, endStation) {
    const travelTimes = this.trips.get(startStation).get(endStation)
    return travelTimes.reduce((acc, t) => acc + t) / travelTimes.length
  }
}
