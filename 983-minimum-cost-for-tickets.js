...
const mincostTickets = (days, costs) => {
  const numDays = Math.max(...days)
  const dp = new Array(numDays).fill(0)

  const [daily, weekly, monthly] = costs

  let nextDay = 0
  for (let day = 0; day < numDays; day++) {
    if (days[nextDay] - 1 === day) {
      nextDay++
      dp[day] = Math.min(
        daily + (day >= 1 ? dp[day - 1] : 0),
        weekly + (day >= 7 ? dp[day - 7] : 0),
        monthly + (day >= 30 ? dp[day - 30] : 0)
      )
    } else {
      dp[day] = day >= 1 ? dp[day - 1] : 0
    }
  }
  return dp.at(-1)
}
