...
const numOfMinutes_dfs = (n, headID, manager, informTime) => {
  ...
  const dfs = (manager, time) => {
    maxTime = Math.max(maxTime, time)
    for (const employee of team[manager])
      dfs(employee, time + informTime[manager])
  }
...
const numOfMinutes_bfs = (n, headID, managers, informTime) => {
  ...
  for (let employee = 0; employee < n; employee++) {
    if (managers[employee] !== -1) {
      team[managers[employee]].push(employee)
    }
  }
  ...
  while (queue.length > 0) {
    ...
    maxTime = Math.max(maxTime, time)
    for (const employee of team[manager])
      queue.push([employee, time + informTime[manager]])
  }
