...
const jobScheduling_topDownDP = (starts, ends, profits) => {
  ...
  for (let i = 0; i < length; i++) starts[i] = jobs[i].start
...
const jobScheduling_bottomUpDP = (starts, ends, profits) => {
  ...
  for (let job = length - 1; job >= 0; job--) {
    ...
    profit += nextJob === length ? 0 : memo[nextJob]
    memo[job] = Math.max(profit, memo[job + 1] ?? 0)
  }
...
const jobScheduling = (starts, ends, profits) => {
  ...
  const jobQueue = new MinPriorityQueue({ priority: ({ end }) => end })
