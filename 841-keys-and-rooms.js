 ...
 * @return {boolean}
 */
const canVisitAllRooms_DFSarray = (rooms) => {
  ...
  while (stack.length) {
    ...
    visited[room] = true
    for (const key of rooms[room]) if (!visited[key]) stack.push(key)
  }

  return visited.every((room) => room)
}

// const canVisitAllRooms_DFSset = (rooms) => {
const canVisitAllRooms = (rooms) => {
  ...
  while (stack.length) {
    const room = stack.pop()
    visited.add(room)
    for (key of rooms[room]) if (!visited.has(key)) stack.push(key)
  }
