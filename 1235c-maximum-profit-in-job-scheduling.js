...
const jobScheduling_topDownDP = (starts, ends, profits) => {
  ...
  const findMaxProfit = (job) => {
    ...
    const nextJob = findNextJob(end)
    const maxProfit = Math.max(
      profit + findMaxProfit(nextJob),
      findMaxProfit(job + 1)
    )
    return (memo[job] = maxProfit)
  }
...
const jobScheduling = (starts, ends, profits) => {
  ...
  const queue = new MinPriorityQueue({ priority: (job) => job.end })

  let maxProfit = 0

  for (const { start, end, profit } of jobs) {
    while (!queue.isEmpty() && queue.front().priority <= start)
      maxProfit = Math.max(maxProfit, queue.dequeue().element.profit)
    queue.enqueue({ end, profit: profit + maxProfit })
  }
