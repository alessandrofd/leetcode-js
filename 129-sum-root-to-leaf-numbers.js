  ...
  while (root) {
    if (root.left) {
      ...
      while (predecessor.right && predecessor.right !== root) {
        ...
        // Cálculo do comprimento do ramo que antecede a raiz
        steps++
      }

      if (!predecessor.right) {
        // 1a. travesia
        // Navegação à esquerda e descendente
        num = num * 10 + root.val
        predecessor.right = root
        root = root.left
      } else {
        // predecessor.right == root - 2a. travesia
        if (!predecessor.left) sum += num
        predecessor.right = null
        root = root.right
        // Descarte dos dígitos do ramo já percorrido
        // O dígito da raiz é preservado
        num = Math.floor(num / 10 ** steps)
      }
    } else {
      // Navegação à direita e possivelmente ascendent
      num = num * 10 + root.val
      if (!root.right) sum += num
      root = root.right
    }
  }
  return sum
