...
const minSpeedOnTime = (dist, hour) => {
  ...
  const time = (speed) => {
    let t = 0
    for (let i = 0; i < n - 1; i++) t += Math.ceil(dist[i] / speed)
    t += dist.at(-1) / speed
    return t
  }

  const sumDist = dist.reduce((acc, d) => acc + d)
  let lo = Math.max(1, Math.floor(sumDist / hour))
  let hi = Math.ceil(sumDist / (hour - (n - 1)))

  while (lo < hi) {
    const mid = Math.floor((lo + hi) / 2)
    if (time(mid) > hour) lo = mid + 1
    else hi = mid
  }
