 ...
 * todos os nós e exploramos os nós adjacentes. Os nós representando água são
 ...
 * Assim que atingirmos o outra ilha, retornamos a distância.
 * envolve DFS.
 */
...
const shortestBridge = (grid) => {
  ...
  let startRow = 0
  let startCol = 0
  findLand: for (startRow = 0; startRow < n; startRow++)
    for (startCol = 0; startCol < n; startCol++)
      if (grid[startRow][startCol] == 1) break findLand
  ...
  const markIsland = (row, col) => {
    ...
    for (const [deltaRow, deltaCol] of deltas)
      markIsland(row + deltaRow, col + deltaCol)
  }

  markIsland(startRow, startCol)
  ...
  while (queue.length) {
    ...
    for (const [row, col] of queue) {
      for (const [deltaRow, deltaCol] of deltas) {
        ...
        if (nextRow < 0 || nextCol < 0 || nextRow >= n || nextRow >= n) continue
