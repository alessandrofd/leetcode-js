...
const longestObstacleCourseAtEachPosition = (obstacles) => {
  ...
  const binSearch = (heights, target) => {
    let left = 0
    let right = heights.length
    while (left < right) {
      const mid = Math.floor((right + left) / 2)
      if (heights[mid] <= target) left = mid + 1
      else right = mid
    }
    return left
  }

  for (const obstacleHeight of obstacles) {
    if (heights.length == 0 || obstacleHeight >= heights.at(-1)) {
      heights.push(obstacleHeight)
      result.push(heights.length)
    } else {
      const circuitLength = binSearch(heights, obstacleHeight)
      heights[circuitLength] = obstacleHeight
      result.push(circuitLength + 1)
    }
  }
