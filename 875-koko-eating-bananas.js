...
const minEatingSpeed = (piles, h) => {
  ...
  /**
   * O alvo desta busca binária (mid) é o tamanho de pilha de bananas imediata-
   * mente menor que o tamnho mínimo para que todas as pilhas sejam consumidas
   * no tempo dado. Na última iteração lo é acrescido de um e chegamos à solução
   */
  lo = 1
  hi = Math.max(...piles)
  while (lo < hi) {
    const mid = Math.floor((lo + hi) / 2)
    if (pileBigEnough(mid)) hi = mid
    else lo = mid + 1
  }
  return lo
}
